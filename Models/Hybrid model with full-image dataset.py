...
vgg_model = VGG16(input_shape=(289, 188,3), weights='imagenet', include_top=False,pooling='avg')
...
train_features = vgg_model.predict(train_data, verbose= 1)


# In[19]:


val_features = vgg_model.predict(val_data, verbose= 1)


# In[19b]: CORRECTED - extract features for the held-out test set too

test_features = vgg_model.predict(test_data, verbose= 1)
...
# This is the specific comparison the Discussion section makes a claim about (full-image
# hybrid vs. split-image hybrid), so it needs to be a trustworthy, genuinely held-out number.
